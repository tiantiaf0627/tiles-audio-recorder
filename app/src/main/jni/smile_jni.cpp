...
JNIEXPORT jstring JNICALL Java_com_audeering_opensmile_androidtemplate_SmileJNI_SMILExtractJNI( JNIEnv* env, jobject thiz, jstring conf, jint updateProfile, jstring output){
  ...
  // Starts openSMILE through JNI.
 JNIEXPORT jstring JNICALL Java_com_audeering_opensmile_androidtemplate_SmileJNI_SMILExtractWavJNI( JNIEnv* env, jobject thiz, jstring conf, jint updateProfile, jstring input, jstring output){
     const char SMILExtract[] = "SMILExtract";
     const char * configfile = env->GetStringUTFChars(conf, NULL);
     ...
     // -l <N> sets the debugging level. 5 is most verbose. 0 supresses all messages. 2 is recommended.
     // -O option is a dummy, no actual output file should be written to this location
     const char *cmd[] = {SMILExtract, "-C", configfile, "-nologfile", "-l", "2", "-I", inputfile, "-O", outputfile};
     // TODO: sdcard location, if needed: Environment.getExternalStorageDirectory()
     int cmd_num = 10; // IMPORTANT: synchronize with the size of the cmd array
     // NOTE: app will seg-fault if cmd_num is larger than the actual number of arguments!
 ...
 // Starts openSMILE through JNI.
  JNIEXPORT jstring JNICALL Java_com_audeering_opensmile_androidtemplate_SmileJNI_SMILExtractWavAndFeatureJNI( JNIEnv* env, jobject thiz, jstring conf, jint updateProfile, jstring output, jstring lldoutput, jstring waveoutput){
      const char SMILExtract[] = "SMILExtract";
      const char * configfile = env->GetStringUTFChars(conf, NULL);
      const char * outputfile = env->GetStringUTFChars(output, NULL);
      const char * lldcsvoutput = env->GetStringUTFChars(lldoutput, NULL);
      const char * wavoutputfile = env->GetStringUTFChars(waveoutput, NULL);

      //"-nologfile" necessary, as we cannot write to the default logfile location!
      // -l <N> sets the debugging level. 5 is most verbose. 0 supresses all messages. 2 is recommended.
      // -O option is a dummy, no actual output file should be written to this location
      const char *cmd[] = {SMILExtract, "-C", configfile, "-O", outputfile, "-W", wavoutputfile, "-nologfile", "-l", "2"};
      // TODO: sdcard location, if needed: Environment.getExternalStorageDirectory()
      int cmd_num = 10; // IMPORTANT: synchronize with the size of the cmd array
      // NOTE: app will seg-fault if cmd_num is larger than the actual number of arguments!
 ...
 // Starts openSMILE through JNI.
 JNIEXPORT jstring JNICALL Java_com_audeering_opensmile_androidtemplate_SmileJNI_SMILExtractOutputJNI( JNIEnv* env, jobject thiz, jstring conf, jint updateProfile, jstring output){
     const char SMILExtract[] = "SMILExtract";
     const char * configfile = env->GetStringUTFChars(conf, NULL);
     const char * outputfile = env->GetStringUTFChars(output, NULL);
     //"-nologfile" necessary, as we cannot write to the default logfile location!
 ...
 // Starts openSMILE through JNI.
  JNIEXPORT jstring JNICALL Java_com_audeering_opensmile_androidtemplate_SmileJNI_SMILEVADJNI( JNIEnv* env, jobject thiz, jstring conf, jint updateProfile){
      const char SMILExtract[] = "SMILExtract";
      const char * configfile = env->GetStringUTFChars(conf, NULL);
